### CNN Classifier - PyTorch Implementation
...
def train_CNN(X, y, params=None, retrain=False, save_accuracy=True, save_model_flag=False):
    '''
        ...
        Function trains and returns a pytorch CNN model with given params
    '''
    ...
    save_dir = f"{SAVED_MODELS_DIR}/{PYTORCH_CNN_MODEL_NAME}/"
...
save_dir = f"{SAVED_MODELS_DIR}/{PYTORCH_CNN_MODEL_NAME}/"
save_model = save_dir + "best_pytorch_CNN_model"
...
values_calculated = True # set to False to train
if not values_calculated:
    ...
    # loop through p values between 0 - 1
    dropout_p_list = [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 0.99]  
...
save_file = f"{SAVED_MODELS_DIR}/{PYTORCH_CNN_MODEL_NAME}/{RESULTS_FILE}"
...
plt.savefig(f"{PLOTS_DIR}/pytorch_CNN_validation.pdf")
plt.clf()
...
if loss_flag:
    ...
    plt.savefig(f"{PLOTS_DIR}/pytorch_CNN_loss.pdf")
    plt.clf()
...
## Get test accuracy -- either calculate if not yet done (only if GPU)
